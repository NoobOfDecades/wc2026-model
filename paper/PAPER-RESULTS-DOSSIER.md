...
The project ran an append-only, publicly numbered error ledger throughout; corrections are new dated changelog entries, never edits. **Count: 19 numbered entries plus 1 disclosed, unnumbered proof-standard upgrade** [count corrected 20 Jul: the write-up drafts stated 18 numbered; the sealed changelog runs #1 through #19, so 19 is the numbered count - the honest figure, taken even though it is the less flattering read of my own ledger] (the live dossier count governs at write-up; the "twelve"/"seventeen" figures in earlier framing text are superseded slips, never downgraded to match a slip).
